...
async function runJsonPerOp(n) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    try {
      ...
      // On Windows, rename can fail if file is locked; fallback to writing the target directly
...
async function run() {
  ...
  if (mode === 'both' || mode === 'sqlite') {
    ...
    if (!res) {
      ...
      console.log(`  per-op inserts: ${res.perTime.toFixed(02)} ms`);
      console.log(`  batch inserts (transaction): ${res.batchTime.toFixed(02)} ms`);
      console.log(`  read/count: ${res.readTime.toFixed(02)} ms (rows=${res.count})`);
    }
  }
