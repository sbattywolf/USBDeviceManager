...
function Invoke-AgentMonitoringTests {
    ...
    Write-Output "Starting SimRacing Agent Monitoring Unit Tests"
    Write-Output "============================================="
    Write-Output ""

    try {
        foreach ($suite in $TestSuites) {
            Write-Output "Running agent monitoring test suite: $suite"
            ...
            $allResults += $result
                ...
                if ($StopOnFirstFailure) {
                    Write-Error "Stopping agent monitoring tests due to failure in $suite"
                    break
                }
            }
        ...
        # Summary - aggregate per-suite Summary objects returned from each test suite
        ...
        Write-Output "Agent Monitoring Test Summary"
        Write-Output "============================"
        Write-Output "Total Passed:  $totalPassed"
        if ($totalFailed -gt 0) { Write-Error "Total Failed:  $totalFailed" } else { Write-Output "Total Failed:  $totalFailed" }
