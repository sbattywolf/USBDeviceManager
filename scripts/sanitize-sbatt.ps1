...
Write-Host "Scanning repository for matches to pattern: $search"

# Use git to find tracked files containing the pattern (case-insensitive)
$files = & git grep -Il --line-number -e 'sbatt' 2>$null | ForEach-Object { ($_ -split ":",2)[0] } | Sort-Object -Unique
...
foreach ($f in $files) {
    ...
    $new = [regex]::Replace($content, $search, $replacement)
    if ($content -ne $new) {
...
foreach ($f in $files) {
    ...
    $patched = [regex]::Replace($orig, $search, $replacement)
    if ($orig -ne $patched) {
...
& git add -A
$commitMsg = "ci(sanitize): replace sbatt -> Sbatta"
